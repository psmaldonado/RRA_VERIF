...
EXPNAME='QFX0D40M_IDEAL_CONSTANTPAR0.8_MEMNC';
EXPNAMEFNL='TRUE_RUN_SINLAKU_CONSTANT_QFX0.8_60KM';
ENSSIZE=40;
INIT_FREC=6;
...
while(curd <= endd)
      ...
      [USPRD(:,:,:) VSPRD(:,:,:) QSPRD(:,:,:) HSPRD(:,:,:) TSPRD(:,:,:) ...
       U10SPRD(:,:) V10SPRD(:,:) PSPRD(:,:) MCAPESPRD(:,:) MCINSPRD(:,:)]=read_arwpost_analysis(file,nx,ny,nz,true); 
 
       file=[BASEURL '/' EXPNAME '/anal/' dateinit '/plev_anal_mean.dat' ]
      [U(:,:,:) V(:,:,:) Q(:,:,:) H(:,:,:) T(:,:,:) ...
       U10(:,:) V10(:,:) P(:,:) MCAPE(:,:) MCIN(:,:)]=read_arwpost_analysis(file,nx,ny,nz,true);
      ...
      datefile  =datestr(curd ,'yyyy-mm-dd_HH:MM');
      file=[BASEURL '/' EXPNAMEFNL '/plev_' datefile '.dat']
      [UFNL(:,:,:) VFNL(:,:,:) QFNL(:,:,:) HFNL(:,:,:) TFNL(:,:,:) ...
       U10FNL(:,:) V10FNL(:,:) PFNL(:,:) MCAPEFNL(:,:) MCINFNL(:,:)]=read_arwpost_analysis(file,nx,ny,nz,false); 
      %end
