...
while(curd <= endd)
      ...
      [USPRDtmp(:,:,:) VSPRDtmp(:,:,:) QSPRDtmp(:,:,:) HSPRDtmp(:,:,:) TSPRDtmp(:,:,:) ...
       U10SPRD(:,:) V10SPRD(:,:) PSPRD(:,:) MCAPESPRD(:,:) MCINSPRD(:,:)]=read_arwpost_analysis_control(file,nx,ny,14,true); 
 
       file=[BASEURL '/' EXPNAME '/anal/' dateinit '/plev_anal_mean.dat' ];
      [Utmp(:,:,:) Vtmp(:,:,:) Qtmp(:,:,:) Htmp(:,:,:) Ttmp(:,:,:) ...
       U10(:,:) V10(:,:) P(:,:) MCAPE(:,:) MCIN(:,:)]=read_arwpost_analysis_control(file,nx,ny,14,true);
