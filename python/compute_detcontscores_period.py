...
DATE_INIT = '12'            #For date option, specify a single forecast intialization. If empty, FCST_INITS are use.
...
for order in ORDER_BY:
   ...
   if order == 'date':
      scores = ut.init_dict(dict_key1, None)
   else:
      scores = ut.init_dict(dict_key1, dict_key2)
   ...
   for finit in dict_key1:
      ...
      for flead in dict_key2:
         if filelist[finit][flead]:   #Skip if there is no data
            ...
            scores[finit][flead] = detscores.det_cont_fct(fcst, imerg, scores=["RMSE", "ME"])

   # Write data to npz file
   print('--------------------------------------')
   print('Writing GRID-POINT CATEGORICAL SCORES ')
   print('--------------------------------------')
   fileout = OUTDIR + '/det_cont_scores_' + order
