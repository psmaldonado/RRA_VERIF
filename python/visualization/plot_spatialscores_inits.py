...
def plt_env(fig, ax, opts):
   ...
   # Y-axis 
   ax[0].set_ylabel('FSS', fontsize=fs)
   ax[0].set_ylim([0 ,1])
   ax[0].set_yticklabels(yticks,fontsize=fs)

   # Subplot title and grid
   for i, iax in enumerate(ax):
      iax.grid(which='both', color='lightgray')
      iax.text(-0.1, 0.98, letters[i], horizontalalignment='left', verticalalignment='top', fontsize=fstitle)
      iax.set_title(opts['inits'][i] + ' UTC initialization', fontsize=fstitle)
      
   ax[2].set_title('00 and 12 UTC initialization ', fontsize=fstitle)
...
plot_opts = {'colors':['black', 'gray', 'red', 'lightgrey'],
        ...
        'inits': ['00', '12', 'all'],
        'leads': fcst_leads,
        'letters': utcom.get_alphabet(3, 'bracket2')
}
...
for ls in plot_opts['linestyles'][:len(THRESHOLDS)]:
   ax[0].plot([], [], color=plot_opts['colors'][3], linewidth=plot_opts['lw'], linestyle=ls)
# Extra lines for exps
for ms in ['o','s']: #[8,4]:#plot_opts['linestyles'][:len(THRESHOLDS)]:
   ax[0].plot([], [], color='lightgray', linewidth=plot_opts['lw'], linestyle='-', marker=ms)#'o', markersize=ms)
...
labels = ['DA', 'NoRegDA', '880km box-size', '80km box-size', '1mm thr', '25mm thr']# '  5mm thr', '10mm thr', '25mm thr']
...
fig, ax = plt_fss('leads', MODELS['gfs']['leads'], 0.7, '--', fig, ax, plot_opts)

# Legend
# Extra lines for exps
for ls in plot_opts['linestyles'][:len(THRESHOLDS)]:
   ax[0].plot([], [], color='lightgray', linewidth=plot_opts['lw'], linestyle=ls)
# Extra lines for exps
for ms in ['o','s']: #[8,4]:#plot_opts['linestyles'][:len(THRESHOLDS)]:
   ax[0].plot([], [], color='lightgray', linewidth=plot_opts['lw'], linestyle='-', marker=ms) #'o', markersize=ms)
# Extra lines for thresholds
...
lines = ax[0].get_lines()
labels = ['DA', 'NoRegDA', '6-hr fcst', '18-hr fcst', '1mm thr', '25mm thr']# '  5mm thr', '10mm thr', '25mm thr']
idx = np.arange(-6,0).tolist()
ax[2].legend([lines[i] for i in idx], labels,loc=4, handlelength=1.8, prop={'size': plot_opts['fs']}, edgecolor='k', ncol=1)#, bbox_to_anchor=(1.55, 0.6))
