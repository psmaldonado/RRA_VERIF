...
def intensity_scale_3d(X_f, X_o, name, thrs, scales=None, ensemble=False, wavelet="Haar"):
    ...
    intscale = intensity_scale_init(name, thrs, scales, ensemble, wavelet)
    if ensemble:
       intensity_scale_accum(intscale, X_f[0,:,:,:], X_o[0,:,:])
    else:
       intensity_scale_accum(intscale, X_f[0,:,:], X_o[0,:,:])

    for i in range(1,X_f.shape[0]):
       intscale2 = intensity_scale_init(name, thrs, scales, ensemble, wavelet)
       if ensemble:
          intensity_scale_accum(intscale, X_f[i,:,:,:], X_o[i,:,:])
       else:
          intensity_scale_accum(intscale, X_f[i,:,:], X_o[i,:,:])

       intscale = intensity_scale_merge(intscale, intscale2)
...
def intensity_scale_init(name, thrs, scales=None, ensemble=False, wavelet="Haar"):
    ...
    for i, thr in enumerate(intscale["thrs"]):
        ...
        if name.lower() == "bmse":
            ...
            for j, scale in enumerate(intscale["scales"]):
                intscale[thr][scale] = fss_init(thr, scale, ensemble)

    if name.lower() == "fss":
        intscale["ensemble"] = ensemble
...
def fss(X_f, X_o, thr, scale, ensemble=False):
    """
...
def fss_init(thr, scale, ensemble=False):
